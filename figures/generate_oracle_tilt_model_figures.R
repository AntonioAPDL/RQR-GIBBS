...
if (identical(receipt$schema_version, v3_schema)) {
  ...
  if (!all(expected_v5)) {
    oti_stop("The corrected version-5 evidence record is inconsistent.")
  }
}
...
if (identical(receipt$schema_version, v3_schema)) {
  ...
  warning_label <- "Lower bulk ESS in one component"
...
disposition_label <- ifelse(
  disposition == "strict_pass", "No numerical exception", warning_label
)
rows <- sprintf(
  "%s & %s & %.3f & %.3f & %.3f & %.3f & %s \\\\",
