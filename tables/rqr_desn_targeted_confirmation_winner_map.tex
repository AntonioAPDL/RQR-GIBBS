% Generated from RQR-DESN calibration-summary winner_map.csv.
\begin{table}[!htbp]
\centering
\begingroup
\TableStyle
...
\toprule
Stage & Mechanism & $c$ & Selected endpoint specification & Interval score & Coverage & $\Delta$ score & Coverage class \\
\midrule
...
\end{tabular}
\endgroup
\caption{RQR--DESN supplementary calibration summaries. The coverage target is $c$. Selected endpoint specifications are reported as design/prior/learning-rate. $\Delta$ score is the paired interval-score difference relative to the empirical train-interval baseline, so negative values favor the RQR--DESN specification. Coverage classes use the prespecified thresholds, with green indicating absolute coverage error at most 0.05 and yellow at most 0.075.}
