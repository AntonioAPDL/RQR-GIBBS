...
cat >"$control_root/coordinator_status.tsv" <<EOF
field	value
schema_version	rqrgibbs_dlm_affected_wave_coordinator/1.0.0
...
update_status running s10_guard
...
update_status running oracle_reference
...
update_status running task_preparation
env "${common_env[@]}" Rscript \
...
update_status running affected_wave_workers
pids=()
...
if (( worker_failure != 0 )); then
  update_status failed affected_wave_workers
  exit 1
fi

update_status running compact_closeout
env "${common_env[@]}" Rscript \
