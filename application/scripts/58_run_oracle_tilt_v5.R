...
if (!length(script_path) || is.na(script_path)) {
  script_path <- "application/scripts/58_run_oracle_tilt_v5.R"
}
...
confirmation <- switch(
  mode,
  benchmark = c("RQR_ORACLE_TILT_V5_BENCHMARK_CONFIRM", "YES"),
  execute = c("RQR_ORACLE_TILT_V5_CONFIRM", "YES"),
  NULL
)
...
default_output <- file.path(
  repo_root, "application", "outputs", "oracle_tilt_c095_v5_exact_delta",
...
source_state <- list(
  schema_version = otv5_schema(), mode = mode,
...
preflight <- otv5_design_preflight(config)
if (!preflight$pass) oti_stop("The corrected V5 design preflight did not pass.")
...
if (identical(mode, "preflight")) {
  ...
  message("[oracle-tilt-v5] preflight passed: ", output_root)
  quit(save = "no", status = 0L)
}

if (identical(mode, "reference-only")) {
  reference <- otv5_reference_suite(config)
  otf_atomic_write_csv(reference, file.path(output_root, "reference_gates.csv"))
  passed <- nrow(reference) == 36L && all(reference$pass)
  close_and_manifest(list(
  ...
  message("[oracle-tilt-v5] reference suite passed: ", output_root)
  quit(save = "no", status = 0L)
}
...
required_bundles <- if (identical(mode, "benchmark")) {
  c(preflight = "RQR_ORACLE_TILT_V5_PREFLIGHT_DIR",
    `reference-only` = "RQR_ORACLE_TILT_V5_REFERENCE_DIR")
} else {
  c(preflight = "RQR_ORACLE_TILT_V5_PREFLIGHT_DIR",
    `reference-only` = "RQR_ORACLE_TILT_V5_REFERENCE_DIR",
    benchmark = "RQR_ORACLE_TILT_V5_BENCHMARK_DIR")
}
...
if (identical(mode, "benchmark")) {
  ...
  message("[oracle-tilt-v5] representative benchmark passed: ", output_root)
  quit(save = "no", status = 0L)
}

worker_contract_base <- list(
  schema_version = otv5_worker_schema(), source_commit = source_commit,
...
run_worker <- function(family, target, chain) {
  ...
  envelope <- list(
    schema_version = otv5_worker_schema(), contract = contract,
...
validate_cell_bundle <- function(family, target, require_eligible = TRUE) {
  ...
  checks <- c(
    identical(receipt$schema_version, otv5_cell_schema()),
...
write_cell_bundle <- function(family, target, cell, manifest, elapsed) {
  ...
  receipt <- list(
    schema_version = otv5_cell_schema(), source_commit = source_commit,
    config_sha256 = config_sha256, runtime_tree_digest = runtime_digest,
    family = family, target = target,
...
if (identical(execute_stage, "prepare")) {
  ...
  refresh_run_status()
  message("[oracle-tilt-v5] process-isolated execution prepared: ", output_root)
  quit(save = "no", status = 0L)
}
...
if (identical(execute_stage, "cell")) {
  ...
  if (dir.exists(final)) {
    ...
    refresh_run_status()
    message("[oracle-tilt-v5] validated existing cell: ", family, "/", target)
    quit(save = "no", status = 0L)
  }
  ...
  message(
    "[oracle-tilt-v5] isolated cell ", family, "/", target, ": ",
