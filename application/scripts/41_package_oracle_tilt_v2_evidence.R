...
if (!length(script_path) || is.na(script_path)) {
  script_path <- "application/scripts/41_package_oracle_tilt_v2_evidence.R"
}
...
required_closeout <- c(
  identical(closeout$schema_version, otv2_schema()),
...
if (!all(required_closeout)) {
  oti_stop("The run does not satisfy the v2 compact-evidence contract.")
}
...
if (nrow(reference) != 12L || !all(reference$pass) ||
...
files <- setdiff(otv2_compact_files(), "failure_log.csv")
missing <- files[!file.exists(file.path(run_dir, files))]
if (length(missing)) {
  oti_stop("Compact v2 artifacts are missing: ", paste(missing, collapse = ", "))
}
for (file in files) {
  ...
  if (identical(file, "runtime_binding.json")) {
    ...
    oti_stop("Could not stage compact v2 evidence file: ", file)
  }
}
for (file in wrapper_required) {
  if (!file.copy(file.path(run_dir, file), file.path(stage, file),
                 overwrite = FALSE)) {
    oti_stop("Could not stage monitored v2 evidence file: ", file)
  }
}
...
receipt <- list(
  schema_version = "rqrgibbs_oracle_tilt_evidence/2.0.0",
...
writeLines(c(
  "# Oracle-tilt illustration evidence (version 2)",
  "",
  paste(
  ...
  paste(
    "The dynamic construction uses a fixed physical horizon and the exact",
    "continuous-time local-linear transition covariance at each grid step."
  ),
  paste(
...
if (file.exists(output_dir) || dir.exists(output_dir)) {
  ...
  if (!file.rename(output_dir, backup)) {
    oti_stop("Could not preserve the existing v2 evidence directory.")
  }
  message("[oracle-tilt-v2-evidence] preserved prior evidence at: ", backup)
}
dir.create(dirname(output_dir), recursive = TRUE, showWarnings = FALSE)
if (!file.rename(stage, output_dir)) {
  oti_stop("Could not atomically publish compact v2 evidence.")
}
message("[oracle-tilt-v2-evidence] published: ", output_dir)
