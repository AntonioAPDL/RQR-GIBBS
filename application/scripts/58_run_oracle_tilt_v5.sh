...
requested_output="${RQR_ORACLE_TILT_V5_OUTPUT_DIR:-$repo_root/application/outputs/oracle_tilt_c095_v5_exact_delta/${mode}_${timestamp}_${short_sha}}"
...
if [[ -n "$first_entry" && ! ( "$mode" == execute && "${RQR_ORACLE_TILT_V5_RESUME:-}" == YES ) ]]; then
...
if [[ "$mode" == execute ]]; then
  runner=(
    bash application/scripts/58_orchestrate_oracle_tilt_v5_execute.sh
    "--config=application/config/oracle_tilt_c095_v5_exact_delta_20260810.json"
    "--output-dir=$output_dir"
  )
else
  runner=(
    Rscript application/scripts/58_run_oracle_tilt_v5.R
    "--mode=$mode"
    "--config=application/config/oracle_tilt_c095_v5_exact_delta_20260810.json"
    "--output-dir=$output_dir"
  )
...
echo "[oracle-tilt-v5-monitor] $mode passed: $output_dir"
