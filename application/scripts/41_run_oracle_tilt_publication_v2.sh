...
for command_name in setsid ps awk sha256sum stat find sort date Rscript; do
...
requested_output="${RQR_ORACLE_TILT_V2_OUTPUT_DIR:-$repo_root/application/outputs/oracle_tilt_c095_publication_v2/${mode}_${timestamp}_${short_sha}}"
...
if [[ -n "$first_entry" && ! ( "$mode" == execute && "${RQR_ORACLE_TILT_V2_RESUME:-}" == YES ) ]]; then
...
echo "[oracle-tilt-v2-monitor] $mode passed: $output_dir"
