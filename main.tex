...
A pharmaceutical batch-level application then illustrates how the same methods
place endpoints on observed manufacturing data.
\end{abstract}
...
Finally, a pharmaceutical-manufacturing application shows how these procedures
behave on a real batch-level response where endpoint placement has a direct
...
Section~\ref{sec:tcsp} defines the scan-calibrated tolerance procedure and its
finite-sample feasibility condition. Section~\ref{sec:evaluation} gives the confirmatory
validation design, Section~\ref{sec:pharma-application} gives the
pharmaceutical application, and Section~\ref{sec:discussion} concludes.
...
Figure~\ref{fig:tolerance-validation-width-ranges} also marks the population
...
\label{sec:pharma-application}

We next apply the four reported procedures to the pharmaceutical-manufacturing
data of \citet{ZagarMihelic2022PharmaManufacturing}, using the public
\texttt{Laboratory.csv} file \citep{ZagarMihelic2021LaboratoryCsv}. One
observation corresponds to one manufacturing batch. We restrict attention to
product code 23, which gives 187 batches with a common recorded strength and
batch size. This restriction reduces heterogeneity before comparing
distribution-free two-sided intervals.
...
limits. A content-defined two-sided interval is appropriate for this example
because unusually low and unusually high tablet strength may both be relevant
...
Section~\ref{sec:evaluation}. Held-out empirical content is a finite-data
stability check on the withheld batches; it is not a verification of the
nominal tolerance-confidence statement.
...
endpoint.}
\label{tab:pharma-application-primary}
\TableStyle
\input{tables/pharma_application_primary_summary.tex}
\end{table}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/generated/fig06_pharma_application_tensile_intervals.png}
...
\label{fig:pharma-application-primary}
\end{figure}
...
is larger. The supplement repeats the analysis for tablet-core weight relative
