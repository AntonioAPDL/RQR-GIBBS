...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=black,
  pdftitle={Mean-Tilted Intervals: Fixed-Content Geometry and Scan-Calibrated Tolerance Actions},
...
placements and widths. This ambiguity matters for tolerance inference because a
central equal-tailed interval, a mean-preserving interval, and a shortest
contiguous interval answer different statistical questions. We study the
residual-product criterion introduced by Pouplin et al. under the name Relaxed
Quantile Regression. Under regularity conditions, its unrestricted regular
population minimizer is the unique fixed-content interval whose retained mean
equals the population mean; we call this target the mean-preserving interval
(MPI). The mean-tilted interval (MTI) family generalizes MPI by replacing zero
retained-mean balance with a fixed retained-mean offset. Thus \(\delta=0\)
recovers MPI, and nonzero tilts index the remaining admissible contiguous
fixed-content windows, including distribution-specific equal-tailed and
shortest-contiguous intervals.

The shortest-contiguous member motivates a calibrated tolerance action. We use
TCSP for the tolerance-calibrated shortest-path action: a retained count is
chosen from the distribution-free uniform scan statistic, and the reported set
is the shortest closed order-statistic window at that count. This separates the
finite-sample content--confidence certificate from generalized-posterior
endpoint summaries. A fixed-target MTI-ECM calculation is included as a
calibrated generalized-Bayes comparator: it profiles fitted content and
retained-mean tilt over a prespecified grid and applies a direct
Dirichlet-process fixed-interval content screen selected in a separate
calibration run. A prespecified iid validation study at tolerance confidence
\(0.95\) compares TCSP, the calibrated MTI-ECM comparator, Young--Mathew
interpolation, and Wilks intervals across feasible content--sample-size cells
and eight continuous distributions. The design excludes cells where even the
full sample range cannot support the requested two-sided distribution-free
statement.
\end{abstract}
...
uncertainty for a fixed target rather than creating a response-predictive or
finite-sample tolerance guarantee.
...
keeps candidates that satisfy a direct Dirichlet-process fixed-interval content
screen, and applies a width-stability restriction relative to TCSP. This makes
MTI-ECM a calibrated empirical comparator tied to the MTI loss. It does not
replace the distribution-free scan certificate attached to TCSP. A companion
...
The scope is intentionally bounded. The formal tolerance statement in this
...
are loss-based summaries of interval-root functionals, not posterior-predictive
response draws. Proper scoring rules remain useful when their target matches the
...
certification boundary. Section~\ref{sec:evaluation} gives the confirmatory
...
loss-based generalized posterior for a fixed interval target. It is not a
sampling model for responses, and \(\delta\) is fixed before computation rather
than sampled as an ordinary response parameter.
...
\paragraph{Direct content screen.}
The MTI-ECM comparator also uses a response-distribution content screen during
calibration. For a fixed interval \(I\), a direct Dirichlet-process posterior for
\(F\) gives an exact Beta law for \(F(I)\) conditional on the observed sample
\citep{Ferguson1973DP}. This screen is used to select eligible MTI-ECM
