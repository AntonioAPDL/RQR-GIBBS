...
fixed-design regression, with ordinary RQR obtained exactly at zero tilt. A
dynamic linear extension replaces Gaussian coefficient blocks by alternating
root-specific forward-filtering backward-sampling steps: the stacked state
prior is Gaussian, but the joint augmented observation kernel is quartic.
A frozen deep echo-state-network feature matrix supplies a deterministic
nonlinear readout specialization. Current software and empirical evidence
concern ordinary RQR; the nonzero-tilt algorithms are derived but not yet
...
regularized-horseshoe priors provide ordinary-RQR readout specifications
\citep{CarvalhoPolsonScott2010HS,PiironenVehtari2017RHS}. The proposed
nonzero-tilt algorithms initially use proper Gaussian priors, including
ridge priors; the
marginal propriety of the external horseshoe-family adapter under nonzero tilt
is unresolved.
...
deterministic deep echo-state-network (DESN) feature matrix gives a secondary
frozen-feature readout specialization
\citep{Jaeger2001EchoState,GallicchioMicheliPedrelli2018DeepESNDesign}.
The reservoir is a recorded design object, not a posterior state.
...
is the implemented ordinary-RQR sampler; and (iv) a root-blocked dynamic linear
extension plus a frozen-feature DESN adapter. The ordinary fixed-design and
DLM samplers and the frozen-feature DESN readout adapter are implemented.
...
mean-tilted RQR. Section~\ref{sec:posterior} develops fixed-design
generalized-Bayes computation, and Section~\ref{sec:structured} treats the DLM
and frozen-DESN predictor architectures. Section~\ref{sec:evaluation} states
the evaluation strategy, and Section~\ref{sec:discussion} concludes.
...
All panels begin with raw
\(Y\sim\operatorname{AL}_{0.8}(0,1)\) under the quantile-regression
asymmetric-Laplace convention and display
\(Z=\{Y-\E(Y)\}/\operatorname{SD}(Y)\), with interval content \(c=0.80\). The
subscript is the distribution's quantile/asymmetry index
\(\tau_{\mathrm{AL}}=0.8\), so
\(\Pr(Y\leq 0)=\tau_{\mathrm{AL}}\). This index is specified independently of
the interval content and was not chosen to match it; the common numerical
value \(0.8\) is incidental to this illustration.
All targets are computed on the raw population scale; their display
coordinates then subtract \(\E(Y)\) and divide by
\(\operatorname{SD}(Y)\). Equal-tailed (ET)
...
solid. This population benchmark is distinct from the pseudo-asymmetric-Laplace
augmentation of the residual product in Section~\ref{sec:gibbs}. These
deterministic population calculations compare the functionals; they do not
provide finite-sample or comparative-performance evidence.}
...
For the same raw \(Y\sim\operatorname{AL}_{0.8}(0,1)\), displayed after
mean/standard-deviation standardization, and separately specified content
\(c=0.80\), the
...
\section{Generalized-Bayes Computation under Fixed Design}
\label{sec:posterior}

\subsection{Fixed-design target and model-family status}
...
& Proper Gaussian (including ridge); ordinary external shrinkage adapter
& Gaussian coefficient draw\\
DLM
& \(\eta_{kt}=\vect f_t^\top\vect\theta_{kt}\)
& Fixed \(W_t\), frozen template, or ordinary shared component scales
& Root-specific FFBS\\
Frozen DESN
& \(\eta_k=\mat H_{\mathrm{DESN}}\vect\beta_k\)
& Recorded features; ridge or ordinary readout adapter
& Fixed-design Gaussian draw\\
\bottomrule
\end{tabularx}
...
& Implemented ordinary readout adapter\\
MT-RQR-DESN
& Fixed only
...
\begin{algorithmblock}{Fixed-rate MT-RQR fixed-design Gibbs scan}
...
\(\vect\delta\), and proper Gaussian root priors, including ridge priors. The
scan applies to
fixed, recorded design rows; it does not define a response likelihood.
\item \textbf{Inputs and fixed quantities.} Supply
\(\vect y,\mat X,\mathcal O,c,\omega_{\mathrm R},\vect\delta_{\mathcal O}\),
the two prior canonical parameter pairs, iteration controls, and a recorded
label-swap rule. Missing rows are excluded from every loss, latent-scale, and
tilt site.
...
conditional;
\item draw \(\vect\beta_1\) conditional on \(\vect\beta_2\) from its Gaussian
system with shift
...
\(\vect\beta_1\), using the same shift; and
...
Zero tilt is implemented and validated; nonzero tilt is derived but not yet
...
conditional-Gaussian shrinkage adapter may update separate root-specific
states. That external adapter remains ordinary-only: neither nonzero-tilt
propriety nor learned-rate normalization has been established for it.
...
response density in \(y_i\). Here \(\vartheta\) collects both roots. If
...
\section{Structured Predictor Extensions}
\label{sec:structured}

\subsection{Dynamic linear root states}
\label{sec:dynamic}
...
Let \(\mat H_{\mathrm{DESN}}\) be a deterministic feature matrix produced by a
deep echo-state network with fixed, recorded reservoir weights, scaling,
washout, preprocessing, and random seeds
\citep{Jaeger2001EchoState,GallicchioMicheliPedrelli2018DeepESNDesign,
McDermottWikle2018DeepESNUQ}. Setting
\(\mat X=\mat H_{\mathrm{DESN}}\) in
Algorithm~\ref{alg:fixed-design-gibbs} defines the fixed-rate MT-RQR-DESN
readout. Thus this architecture is a fixed-design specialization, not a third
loss or a posterior model for the reservoir. A proper ridge readout supports
the proposed nonzero-tilt conditional Gaussian blocks. The current external
regularized-horseshoe adapter and its evidence concern ordinary RQR only;
nonzero-tilt propriety and implementation remain open.

Future root functionals use a frozen future feature matrix
\(\mat H_{\mathrm{DESN,fut}}\) constructed under the same recorded reservoir
contract. They are ordered endpoint, midpoint, and width functions of the
readout draws. No response-simulation distribution is implied. Reservoir
selection or feature tuning must occur outside the generalized-posterior scan
and be recorded as part of the design protocol.
...
templates, and ordinary-RQR shared component scales define fixed-joint
targets. The nonzero-tilt component-scale interweaving extension remains
disabled. Recomputing discount covariances from each changing conditional
filter remains a working recursion, not an exact Gibbs sampler for a
demonstrated joint density.
