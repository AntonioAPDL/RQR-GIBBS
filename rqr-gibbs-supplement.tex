...
protocol. Endpoint regression, basis-expansion endpoint models, time-varying endpoint
...
path tolerance procedure. The generalized posterior is always a loss-based update
...
Symbol & Meaning & Use\\
\midrule
\(c\) & requested population content & fixed-content targets and tolerance procedures\\
...
& Section~\ref{sec:supp-pharma-application}
& Records data provenance, response diagnostics, supplementary RSD results, and held-out sensitivity checks\\
Validation limits
...
Figure~\ref{fig:supp-tolerance-population-shortest}. For each simulation law
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/generated/figS03_tolerance_population_shortest_intervals.png}
...
article.}
\label{fig:supp-tolerance-population-shortest}
\end{figure}
...
\label{sec:supp-pharma-application}

The pharmaceutical application uses the public manufacturing data described by
\citet{ZagarMihelic2022PharmaManufacturing}. The analysis file is
\texttt{Laboratory.csv}, version 1 on Figshare
\citep{ZagarMihelic2021LaboratoryCsv}. The downloaded file contains 1005
...
\label{tab:supp-pharma-response-diagnostics}
\TableStyle
\input{tables/pharma_application_response_diagnostics.tex}
\end{table}
...
sample; the rule itself is not tuned on these pharmaceutical responses.

\begin{table}[H]
\centering
\caption{\textbf{Supplementary weight-RSD application.} Intervals are fit on
...
\label{tab:supp-pharma-rsd}
\TableStyle
\input{tables/pharma_application_supplement_summary.tex}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/generated/figS04_pharma_application_rsd_intervals.png}
...
experiment. The summaries in Table~\ref{tab:supp-pharma-dependence} indicate
...
\label{tab:supp-pharma-dependence}
\TableStyle
\input{tables/pharma_application_dependence_sensitivity.tex}
\end{table}
