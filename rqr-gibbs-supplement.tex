...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=black,
  pdftitle={Supplementary Material: Mean-Tilted Intervals: Fixed-Content Geometry and Scan-Calibrated Tolerance Actions},
...
Fixed-Content Geometry and Scan-Calibrated Tolerance Actions}}
...
\begin{abstract}
\noindent
This supplement gives the mathematical details and validation records behind
the tolerance-interval article. It first develops the fixed-content interval
class, the mean-preserving interval (MPI), and the mean-tilted interval (MTI)
family. It then records the finite-sample score balance used to interpret
fixed-target fits, the pseudo-asymmetric-Laplace construction used by the
MTI-ECM mode calculation, the direct Dirichlet-process fixed-interval content
screen, and the iid tolerance-validation protocol. Endpoint regression,
fixed-feature readouts, time-varying endpoint models, and broader Gibbs
computation are deferred to a companion computation paper.
\end{abstract}
...
\(\gamma\) & posterior content-screen error probability & direct DP screen\\
\(\omega_{\mathrm R}\) & fixed generalized-Bayes learning rate & fixed-target MTI-ECM\\
\(F,H,a\) & response distribution, DP base measure, and DP concentration & content screening\\
...
& Section~\ref{sec:supp-direct-dp-screen}
...
& Lists feasible cells, distributional scenarios, calibration choices, and table sources\\
Evidence boundaries
...
\section*{Part III. Fixed-Target Computation and Content Screening}
...
tolerance comparison to the MTI generalized-Bayes construction. It is not a
response-likelihood EM algorithm and it does not supply the distribution-free
tolerance certificate attached to TCSP.
...
\section{Direct DP Content Screen}
\label{sec:supp-direct-dp-screen}

The MTI-ECM comparator uses a model-based content screen during calibration.
...
The direct DP screen is used only to choose eligible MTI-ECM candidates in the
...
The MTI-ECM content screen and stability rule are selected in a separate
