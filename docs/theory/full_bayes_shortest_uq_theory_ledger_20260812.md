# Full Bayesian Shortest-Interval Uncertainty Theory Ledger
...
| `T-DPM-ECM` | DPM ECM diagnostic | `IMPLEMENTED-AUDIT-PENDING` | Deterministic MAP-style fit with recorded objective trace and monotone backtracking. | Diagnostic/initializer only; it is separate from posterior uncertainty summaries and from the formal tolerance action. |
...
## Bayesian Uncertainty Position

The primary Bayesian uncertainty path for shortest-interval content is:
...
MTI MCMC/ECM remains a generalized-Bayes fixed-target plug-in summary.
Tolerance confidence instead comes from the scan calibration or the stated
Bayesian content-probability calculation.
